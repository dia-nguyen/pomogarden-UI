function DefaultButton({ label, event, classes }) {
  return (
...
function ButtonTimer({ isActive, label, event }) {
  return (
    <DefaultButton
      label={label}
      event={event}
...
function ButtonStartPause({ label, event }) {
  return (
    <DefaultButton
      label={label}
      event={event}
...
function SettingsButton({ event }) {
  return (
    <button onClick={event}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"
        className="w-5 h-5 fill-lime-700 hover:fill-lime-900"
      >
